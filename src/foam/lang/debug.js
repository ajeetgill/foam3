...
foam.CLASS({
  ...
  methods: [
    function validate(model) {
      ...
      var es = foam.lang.Property.SHADOW_MAP || {};
      for ( var key in es ) {
        var e = es[key];
        if ( this[key] ) {
...
foam.SCRIPT({
  ...
  foam.lang.FObject.installModel = function() {
    var superInstallModel = foam.lang.FObject.installModel;
    ...
    return function(m) {
      ...
      for ( var i = 0 ; i < m.axioms_.length ; i++ ) {
        ...
        var Property = foam.lang.Property;
  ...
  foam.lang.FObject.validate = function() {
  ...
  if ( false && globalThis.Proxy ) {
    (function() {
      ...
      var oldCreate = foam.lang.FObject.create;

      foam.lang.FObject.create = function(args, ctx) {
    ...
    foam.__context__ = foam.__context__.createSubContext({
      describe: function() {
        ...
        for ( var key in this ) {
          try {
            var value = this[key];
            var type = foam.lang.FObject.isInstance(value) ?
              value.cls_.name :
              typeof value    ;
            if ( type == 'PropertySlot' ) value = value.get();
            if ( type == 'ConstantSlot' ) value = value.get();
            if ( foam.lang.FObject.isInstance(value) ) value = value.cls_.name;
            this.log(
