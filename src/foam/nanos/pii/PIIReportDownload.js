...
foam.CLASS({
  ...
  requires: [
    'foam.dao.ArraySink',
    'foam.nanos.pii.PIIDisplayStatus',
    'foam.nanos.pii.ViewPIIRequest'
  ],
  ...
  properties: [
    {
      class: 'Enum',
      name: 'requestsStatus',
      of: 'foam.nanos.pii.PIIDisplayStatus',
    },
    {
      class: 'Int',
      name: 'tick',
  ...
  methods: [
    ...
    function render() {
      this.SUPER();
      var self = this;
      var currentUserID = this.subject.user.id;
      ...
      this.tick$.sub(function() {
        if ( self.requestsStatus == self.PIIDisplayStatus.APPROVED ) {
          ...
          .start()
            ...
            .start().addClass('p').add(self.DOWNLOAD_JSON).end()
          .end();
        }
  ...
  actions: [
    {
      ...
      code: function(X) {
        var self = this;
        var vpr = X.window.foam.nanos.pii.ViewPIIRequest.create();
